...
ASL_aoa_df<-read_csv("../RawData/signdata_20231106_updated.csv")%>%
  select("LemmaID", "SignBankLemmaID", "bglm_aoa", "empirical_aoa") %>%
  dplyr::rename(Sign = "SignBankLemmaID")

#Loading English AOA 
WS_aoa_df <- read_csv("../RawData/WS_production_aoa_data.csv") %>%
...
concsFeats <- read_csv("../RawData/MBCDI_concsFeats_2022-07-14.csv") %>%
...
concs1 <- read_csv("../RawData/MBCDI_concs_2022-07-14.csv")  %>%
  subset(Concept %in% prod_aoa_df$Concept)

##Load English frequency information
childes.freqStats <- read_csv("../RawData/MBCDI_CHILDESv0.1.0_freq.csv")  %>%
  subset(Concept %in% prod_aoa_df$Concept)

#Load ASL frequency information 

ASL.freqStats <- read_csv("../RawData/signdata_20231106_updated.csv")%>%
  subset(InCDI=="Yes") %>%
  subset(SignBankLemmaID %in% prod_aoa_df$Sign) %>%
  select("SignBankLemmaID", "SignFrequency(Z)") %>%
  dplyr::rename(Sign = "SignBankLemmaID")


concs <- left_join(concs1, prod_aoa_df, by="Concept") %>%
...
concs<-select(concs,-4:-25,-48)
gdata::keep(columnClasses, concs, sure=TRUE)
save.image(file="../Output/ASLLexFeats.Rdata") 
```
